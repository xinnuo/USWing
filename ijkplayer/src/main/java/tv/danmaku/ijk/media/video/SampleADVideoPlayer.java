...
public class SampleADVideoPlayer extends ListVideoPlayer {
    ...
    protected boolean setUp(List<VideoInfoModel> url, boolean cacheWithPlay, int position, File cachePath, Map<String, String> mapHeadData, boolean changeState) {
        ...
        if (videoModel instanceof ADVideoModel) {
            ADVideoModel adVideoModel = (ADVideoModel) videoModel;
            if (adVideoModel.isSkip() && position < (url.size() - 1)) {
                return setUp(url, cacheWithPlay, position + 1, cachePath, mapHeadData, changeState);
            }
            isAdModel = (adVideoModel.getType() == ADVideoModel.TYPE_AD);
        }
        changeAdUIState();
     ...
     * @return
     */
    public boolean setAdUp(ArrayList<ADVideoModel> url, boolean cacheWithPlay, int position) {
        return setUp((ArrayList<VideoInfoModel>) url.clone(), cacheWithPlay, position);
    }
     ...
     * @return
     */
    public boolean setAdUp(ArrayList<ADVideoModel> url, boolean cacheWithPlay, int position, File cachePath) {
        return setUp((ArrayList<VideoInfoModel>) url.clone(), cacheWithPlay, position, cachePath);
    }
     ...
     * @return
     */
    public boolean setAdUp(ArrayList<ADVideoModel> url, boolean cacheWithPlay, int position, File cachePath, Map<String, String> mapHeadData) {
        return setUp((ArrayList<VideoInfoModel>) url.clone(), cacheWithPlay, position, cachePath, mapHeadData);
    }

    public static class ADVideoModel extends VideoInfoModel {
        /**
         * 正常
         */
         ...
         */
        public ADVideoModel(String url, String title, int type) {
         ...
         * @param isSkip 是否跳过
         */
        public ADVideoModel(String url, String title, int type, boolean isSkip) {
            super(url, title);
            this.mType = type;
            this.isSkip = isSkip;
        }
