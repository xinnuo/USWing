...
public class OrientationUtils {
    ...
    private void init() {
        orientationEventListener = new OrientationEventListener(activity.getApplicationContext()) {
            ...
            public void onOrientationChanged(int rotation) {
                ...
                if (mBaseVideoPlayer != null && mBaseVideoPlayer.isVerticalFullByVideoSize()) {
                    return;
                }
                if (mIsPause) {
                    return;
                ...
                if (((rotation >= 0) && (rotation <= 30)) || (rotation >= 330)) {
                    if (mClick) {
                        ...
                        if (mIsLand > 0) {
                            ...
                            if (mBaseVideoPlayer.getFullscreenButton() != null) {
                                if (mBaseVideoPlayer.isIfCurrentIsFullscreen()) {
                                    mBaseVideoPlayer.getFullscreenButton().setImageResource(mBaseVideoPlayer.getShrinkImageRes());
                                } else {
                                    mBaseVideoPlayer.getFullscreenButton().setImageResource(mBaseVideoPlayer.getEnlargeImageRes());
                                }
                            }
                            mIsLand = 0;
                            mClick = false;
    ...
    public void resolveByClick() {
        if (mIsLand == 0 && mBaseVideoPlayer != null && mBaseVideoPlayer.isVerticalFullByVideoSize()) {
            return;
        }
        mClick = true;
        if (mIsLand == 0) {
    ...
    public int backToProtVideo() {
        if (mIsLand > 0) {
            mClick = true;
            activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
            if (mBaseVideoPlayer != null && mBaseVideoPlayer.getFullscreenButton() != null)
                mBaseVideoPlayer.getFullscreenButton().setImageResource(mBaseVideoPlayer.getEnlargeImageRes());
            mIsLand = 0;
            mClickPort = false;
            return 500;
